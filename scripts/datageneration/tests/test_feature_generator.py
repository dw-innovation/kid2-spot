...
class TestGenerateCombination(unittest.TestCase):
    ...
    def test_property_generate(self):
        entity_id = self.query_comb_generator.descriptor2_ids['restaurant']

        for idx in range(1, 5):
            properties = self.query_comb_generator.generate_properties(entity_id=entity_id, num_of_props=idx)
            assert len(properties) == idx
